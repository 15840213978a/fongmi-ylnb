...
abstract class BaseConfig {
    ...
    protected void loadConfig(int id, Config config, Callback callback) {
        try {
            ...
            if (!TextUtils.isEmpty(notice) && !isErrorNotice(notice)) {
                App.post(() -> Notify.show(notice));
            }
    ...
    private boolean isErrorNotice(String notice) {
        return notice.contains("不存在") || notice.contains("错误") || notice.contains("失败") || notice.contains("无法") || notice.contains("请重载") || notice.contains("请重试") || notice.contains("异常");
    }
