...
public class Activation {
    ...
    private static final String PREF_USED_CODES = "activation_used_codes";
    private static final String SECRET = "ylys_neibu_jz_2024_secret_key";
    private static final String PERMANENT_SECRET = "ylys_permanent_2024_secret";
    ...
    public static boolean activate(String code) {
        ...
        if (isPermanentCode(code)) {
            if (isCodeUsed(code)) return false;
            markCodeUsed(code);
        }
    ...
    public static boolean isCodeAlreadyUsed(String code) {
        code = normalizeCode(code);
        if (code.length() != 30 || !isPermanentCode(code)) return false;
        return isCodeUsed(code);
    }
    ...
    public static boolean validateCode(String code) {
        ...
        try {
            if (isPermanentCode(code)) {
                return validatePermanentCode(code);
            } else {
                return validateTimedCode(code);
            }
        } catch (Exception e) {
            return false;
        }
    ...
    private static boolean isPermanentCode(String code) {
        return code.charAt(0) == 'P';
    }

    private static boolean validatePermanentCode(String code) {
        String idPart = code.substring(0, 8);
        String hmacPart = code.substring(8, 16);
        String expectedHmac = computePermanentHmac(idPart);
        if (!hmacPart.equals(expectedHmac)) return false;
        return true;
    }

    private static boolean validateTimedCode(String code) {
    ...
    private static boolean isCodeUsed(String code) {
        Set<String> used = getUsedCodes();
        return used.contains(code);
    }

    private static void markCodeUsed(String code) {
        Set<String> used = getUsedCodes();
        used.add(code);
        Prefers.put(PREF_USED_CODES, new Gson().toJson(used));
    }

    private static Set<String> getUsedCodes() {
        String json = Prefers.getString(PREF_USED_CODES);
        if (TextUtils.isEmpty(json)) return new HashSet<>();
        try {
    ...
    private static String computePermanentHmac(String idPart) {
        try {
            Mac mac = Mac.getInstance("HmacSHA256");
            mac.init(new SecretKeySpec(PERMANENT_SECRET.getBytes(StandardCharsets.UTF_8), "HmacSHA256"));
            byte[] hash = mac.doFinal(idPart.getBytes(StandardCharsets.UTF_8));
            StringBuilder sb = new StringBuilder();
            for (int i = 0; i < 4; i++) sb.append(String.format("%02x", hash[i] & 0xff));
            return sb.toString();
        } catch (Exception e) {
