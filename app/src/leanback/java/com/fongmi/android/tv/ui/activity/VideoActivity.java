...
public class VideoActivity extends PlaybackActivity implements VodPlaybackHost, CustomKeyDownVod.Listener, TrackDialog.Listener, ParseDialog.Listener, ArrayAdapter.OnClickListener, FlagAdapter.OnClickListener, EpisodeAdapter.OnClickListener, QualityAdapter.OnClickListener, QuickAdapter.OnClickListener, Clock.Callback {
    ...
    protected void onServiceConnected() {
        ...
        if (mPendingDetail != null) {
            mVod.onDetailResult(mPendingDetail);
            mPendingDetail = null;
        }
        if (mPendingPlayer != null) {
            mVod.onPlayerResult(mPendingPlayer);
            mPendingPlayer = null;
        }
        if (mPendingSearch != null) {
            mVod.onSearchResult(mPendingSearch);
            mPendingSearch = null;
        }
        if (mPendingDetail == null && mPendingPlayer == null && mPendingSearch == null) {
            checkId();
        }
    }
    ...
    private View getFocus2() {
        if (mFocus2 != null && mFocus2.getVisibility() == View.VISIBLE && mFocus2 != mBinding.control.action.opening && mFocus2 != mBinding.control.action.ending) return mFocus2;
        View timeBar = getSeekView().findViewById(androidx.media3.ui.R.id.exo_progress);
        return timeBar != null ? timeBar : mBinding.control.action.next;
    }
