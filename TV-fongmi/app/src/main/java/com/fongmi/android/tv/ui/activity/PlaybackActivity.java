...
public abstract class PlaybackActivity extends BaseActivity implements MediaController.Listener, Player.Listener, ServiceConnection {
    ...
    private boolean mPendingUseParse;
    private long mPendingTimeout;
    private long mPendingPosition;
    private MediaMetadata mPendingMetadata;
    ...
    protected void onServiceConnected() {
        ...
        if (mPendingResult != null) {
            startPlayer(mPendingKey, mPendingResult, mPendingUseParse, mPendingTimeout, mPendingPosition, mPendingMetadata);
            mPendingResult = null;
        }
    }
    ...
    protected void startPlayer(String key, Result result, boolean useParse, long timeout, long startPositionMs, MediaMetadata metadata) {
        if (result.getDrm() != null && !FrameworkMediaDrm.isCryptoSchemeSupported(result.getDrm().getUUID())) {
            ...
            mPendingUseParse = useParse;
            mPendingTimeout = timeout;
            mPendingPosition = startPositionMs;
    ...
    private void onControllerConnected() {
        try {
            ...
            if (seekView != null) seekView.setPlayer(mController);
    ...
    private void addSeekListener() {
        ...
        timeBar.addListener(new TimeBar.OnScrubListener() {
            ...
            public void onScrubStop(@NonNull TimeBar timeBar, long position, boolean canceled) {
                if (!canceled && mController != null) {
                    mController.seekTo(position);
                }
                PlaybackActivity.this.setScrubbing(false);
            }
        });
    }
    ...
    private void releaseController() {
        ...
        if (mController != null) {
            PlayerSeekView seekView = getSeekView();
            if (seekView != null) seekView.setPlayer(null);
        }
